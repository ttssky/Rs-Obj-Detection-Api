...
DARKNET_PATH = 'darknet/libdarknet.so' #建立python与darknet的bindings
...
def detect_image(network, class_names, image, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    print("prev dets is {}".format(detections))
    if nms:
    ...
    return sorted(predictions, key=lambda x: x[1]), len(predictions)
