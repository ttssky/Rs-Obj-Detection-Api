...
def inference_proc(category):
    ...
    # 文件目录
    base_dir = os.path.join(dl_segmentation_dir, category, location) #基础目录

    file_base_dir = os.path.join(base_dir, 'files', uid) #生成文件目录
    if not os.path.exists(file_base_dir):
        os.makedirs(file_base_dir)

    weights_base_dir = os.path.join(base_dir, 'weights', seg_model_ver) #模型基础目录
    infer_weights_path = os.path.join(weights_base_dir, seg_name_prefix + \
        '-' + category + '-' + location + '.pth') #模型文件目录
    
    infer_images_dir = os.path.join(file_base_dir, 'images') #从heycloud获取图片目录
    ...
    preprocessing_fn = smp.encoders.get_preprocessing_fn(seg_method, 'imagenet') #图片预处理
    ...
    for _, batch in enumerate(tqdm(test_loader)):
        ...
        pr_mask = model.predict(x_tensor)# 预测
    ...
    if subprocess.call(merge_cmd, shell=True) == 0:
        ...
        return 'Error: polygonize failed!!'
